class J11Fighter():
    ...
    def __init__(self, number, pilot):
        ...
        self.__cur_altitude = 0      #当前飞行高度

    def take_off(self):
        '''让战机起飞，并输出战机信息'''
        self.__cur_altitude = 10000
        print('%s驾驶编号为%s的%s战机从某空军机场起飞，并迅速爬升到%s米高空' %
              (self.pilot, self.number, self.model, self.__cur_altitude))
    
    def launch_missile(self, target):
        '''向目标发射导弹'''
        print('%s战机在%s米高空遭遇%s敌机并向目标发射1枚导弹' %
              (self.model, self.__cur_altitude, target))
...
if __name__ == '__main__':
    ...
    j11.take_off()
    # print(j11.__cur_altitude)
    j11.climb_to(108000)
    # j11.__cur_altitude = 10000
